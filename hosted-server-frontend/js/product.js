...
function fetchPost(id) {
  try {
   fetch(`https://scmarketplace.up.railway.app/posts/get-post?post_id=${id}`,{
      method: 'GET',
    }) 
    .then(response => {
        if(!response.ok){
...
function buildPost(name,email,description,price,images,username) {
  ...
  if(isLoggedIn === 'true')
  {
    ...
    // Call a function to add the current post to bookmarks
    console.log('test')
    addToBookmarks();
...
function addToBookmarks(){
  ...
  // Grab values (make sure user is logged in)
  ...
  try {
      fetch(`https://scmarketplace.up.railway.app/upload-bookmark?user_id=${userId}&post_id=${postId}`,{
          method: "POST"
      })
      .then(response =>{
