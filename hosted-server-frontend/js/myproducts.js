...
{
    try {
        fetch(`https://scmarketplace.up.railway.app/posts/get-all-user-posts?user_id=${id}`,{
            method: 'GET',
        }) 
        .then(response => {
            if(!response.ok){
...
{
    try {
        fetch(`https://scmarketplace.up.railway.app/posts/delete-post?user_id=${userId}&post_id=${id}`,{
            method: "DELETE"
        })
        .then(response => {
            if(!response.ok){
